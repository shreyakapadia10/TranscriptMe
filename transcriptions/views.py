...
def save_response(request, conversation_id, job_id, messages, topics, follow_ups, action_items, questions, members, doc, media_type=None):
	...
	if media_type is None:
		conversation_obj = Conversation(conversation_id=conversation_id, job_id=job_id, credentials=credentials)
	...
	if messages is not None:      
		...
		api_response = symbl.Conversations.get_messages(conversation_id=conversation_id, credentials=credentials)
	...
	if topics is not None:
		...
		api_response = symbl.Conversations.get_topics(conversation_id=conversation_id, credentials=credentials)
	...
	if follow_ups is not None:
		...
		api_response = symbl.Conversations.get_follow_ups(conversation_id=conversation_id, credentials=credentials)
	...
	if action_items is not None:
		...
		api_response = symbl.Conversations.get_action_items(conversation_id=conversation_id, credentials=credentials)
	...
	if questions is not None:
		...
		api_response = symbl.Conversations.get_questions(conversation_id=conversation_id, credentials=credentials)
	...
	if members is not None:
		...
		api_response = symbl.Conversations.get_members(conversation_id=conversation_id, credentials=credentials)
...
# To download files
def download_files(request, conversation_id, job_id=None):
	# if zoom call
	if job_id is None:		
		request_for = request.POST.get('request_for')

		# Insights
		# if zip file is not available
		if request_for == "insights":
			messages = request.POST.get('messages', None)
			action_items = request.POST.get('action_items', None)
			questions = request.POST.get('questions', None)
			topics = request.POST.get('topics', None)
			...
			doc = Document.objects.get(conversation_id=conversation_id)
			
			save_response(request, conversation_id, None, messages, topics, follow_ups, action_items, questions, members, doc, 'zoom call')
		...
		# if zip file is available
			...
			doc = Document.objects.get(conversation_id=conversation_id)
	# if request for media type such as text, audio, video
	else:
		doc = Document.objects.get(job_id=job_id, conversation_id=conversation_id)
...
def transcript_zoom_call(request):
	if request.user.is_authenticated:
		if request.is_ajax():
			...
			meetingId = request.POST.get('meetingId', None)
			...
			doc = Document.objects.create(user=request.user, name=file_name, media_type='zoom call', conversation_id=conversation_id)

			doc.save()
			# save_response(request, conversation_id, None, messages, topics, follow_ups, action_items, questions, members, doc, 'zoom_call')
			response = {
				'msg': 'Success',
			}
			return JsonResponse(response)
